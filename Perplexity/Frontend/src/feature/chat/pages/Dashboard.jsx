...
function extractCodeBlocks(text) {
  // filename bhi detect karta hai jaise // Navbar.jsx ya ### Navbar.jsx
  const regex = /(?:\/\/\s*(\w+\.\w+)|###?\s*(\w+\.\w+))?\s*```(html|jsx|js|css|javascript|react)\n([\s\S]*?)```/gi;
  const blocks = [];
  let match;
  while ((match = regex.exec(text)) !== null) {
    const filename = match[1] || match[2] || null;
    blocks.push({
      lang: match[3].toLowerCase(),
      code: match[4].trim(),
      filename, // Navbar.jsx, Hero.jsx etc
    });
  }
  return blocks;
}
...
// ─── StackBlitz Launcher ──────────────────────────────────────────────────────
function openInStackBlitz(blocks) {
  const jsx = blocks.filter((b) => ["jsx", "react"].includes(b.lang));
  ...
  // React project — multiple files
  if (jsx.length > 0) {
    const files = {
      "src/index.js": `import React from 'react';\nimport ReactDOM from 'react-dom/client';\nimport App from './App';\nimport './index.css';\nReactDOM.createRoot(document.getElementById('root')).render(<App />);`,
      "public/index.html": `<!DOCTYPE html><html><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>App</title></head><body><div id="root"></div></body></html>`,
      "src/index.css": css || "* { margin: 0; padding: 0; box-sizing: border-box; }",
    };

    jsx.forEach((block, i) => {
      if (block.filename) {
        // AI ne filename diya → use karo
        files[`src/${block.filename}`] = block.code;
      } else if (i === 0) {
        // Pehla JSX block → App.jsx
        files["src/App.jsx"] = block.code;
      } else {
        // Baaki blocks → Component1.jsx, Component2.jsx
        files[`src/Component${i}.jsx`] = block.code;
      }
    });

    sdk.openProject({
      title: "AI Generated React App",
      description: "Generated by Aura-4 Ultra",
      template: "create-react-app",
      files,
    }, { newWindow: true, openFile: "src/App.jsx" });
    return;
  }
  ...
  sdk.openProject({
    ...
    template: "html",
    files: {
      "index.html": html || `<!DOCTYPE html><html><head><link rel="stylesheet" href="style.css"/></head><body><script src="index.js"></script></body></html>`,
      "style.css": css || "body { font-family: sans-serif; padding: 2rem; background: #0f0f0f; color: white; }",
      "index.js": js || "",
    },
...
// ─── Code Block UI ────────────────────────────────────────────────────────────
function CodeBlock({ lang, code, isLast }) {
  ...
  return (
    <div className="my-3 rounded-xl overflow-hidden border border-white/8">
      <div className="flex items-center justify-between px-4 py-2 bg-slate-800/80 border-b border-white/5">
        <span className="text-[10px] font-mono text-violet-400 uppercase tracking-widest">{lang}</span>
      ...
      <pre className="overflow-x-auto px-4 py-3 bg-slate-950/80 text-[12px] font-mono text-slate-300 leading-relaxed" style={{ scrollbarWidth: "none" }}>
        <code>{code}</code>
      </pre>
      {isLast && (
        <div className="px-4 py-2.5 bg-slate-900/60 border-t border-white/5 flex justify-end">
          <button onClick={() => openInStackBlitz([{ lang, code }])}
            className="flex items-center gap-2 px-4 py-1.5 rounded-lg text-[11px] font-medium text-white hover:scale-105 active:scale-95 transition-all duration-200"
...
// ─── Smart Renderer ───────────────────────────────────────────────────────────
function SmartRenderer({ text, showCursor }) {
  ...
  const parts = text.split(/```(?:html|jsx|js|css|javascript|react)\n[\s\S]*?```/gi);
    ...
    <span className={showCursor ? "cursor-blink" : ""}>
      {parts.map((part, i) => (
        <span key={i}>
          ...
          {blocks[i] && (
            <CodeBlock lang={blocks[i].lang} code={blocks[i].code} isLast={i === blocks.length - 1} />
          )}
        </span>
      ))}
    </span>
      ...
      isUser
        ...
        >
          {isUser ? (
            ...
            <span className={showCursor ? "cursor-blink" : ""}>
              <div className="prose prose-invert prose-sm max-w-none">
<SmartRenderer text={msg.text} showCursor={showCursor} />
              </div>
            </span>
          )}
        </div>
